...
import { floatsTypes, Status } from '../constants'

export const formatSurplusAndDebtAuctions = (
    auctionsList: (SDKAuction & { status: Status })[],
    userProxy?: string
): IAuction[] => {
    if (auctionsList) {
        ...
        const filteredAuctions: IAuction[] = auctionsList.map(
            (surplusAndDebtAuction: SDKAuction & { status: Status }) => {
                const { auctionDeadline, createdAt, initialBid, createdAtTransaction, biddersList, auctionId, status } =
                    surplusAndDebtAuction

                // if auction is settled, winner is the last bidder
                const winner = biddersList?.[-1]?.bidder || ''
